...
## UI Behavior Spec
- Detailed expected UI behavior is documented in `ui_behavior.md`.
- Use `AGENTS.md` for architecture/process rules and `ui_behavior.md` for concrete UI behavior expectations.
- When behavior changes, update `ui_behavior.md` and corresponding verification tests/screenshots in `verification.md` and `tests/`.
...
### Boundary Condition Settings Behavior

When `Boundary Condition -> Power Source` or `Boundary Condition -> Temperature` is active:

- Items (`Power Source 1..N`, `Temperature 1..N`) are shown in the **Settings** panel only.
- Each item row is expandable/collapsible by clicking either:
  - the row text area
  - the chevron button on the right
- Expanded **Power Source** row shows:
  - `Power (W/m)` input (default `0`)
  - `Assigned Objects` list box
  - `+` and `-` controls for list management
- Expanded **Temperature** row shows:
  - `Temperature (oC)` input (default `0`)
  - `Assigned Surfaces` list box
  - `+` and `-` controls for list management
- Assignment list box UI:
  - fixed height
  - smaller/tighter row text
  - vertical scroll when list exceeds visible height
- Assignment list selection supports:
  - single click (single row)
  - Ctrl/Cmd-click (toggle multi-select)
  - Shift-click (range select)
- `-` removes all currently selected rows in the assignment list.
- `+` opens a placeholder dialog (custom assignment flow not implemented yet).
- Viewport picking:
  - Power Source mode: object-click toggles object assignment.
  - Temperature mode: CAD-face-click toggles surface assignment label (`ObjectName:Face-N`).
- Exclusivity rules:
  - one object can belong to only one power source
  - one surface can belong to only one temperature
  - conflicting assignment attempts are rejected (existing assignment remains).
- Highlight behavior for assignment mode:
  - Power Source: all assigned objects of the active item are highlighted.
  - Temperature: all assigned surfaces of the active item are highlighted (surface-only, not whole-object highlight).
  - no active BC item: assignment highlight is cleared.
...
5. **Boundary Conditions** — expand Boundary Condition in the tree. Under it, click Power Source or Temperature to add items (Power Source 1, Temperature 1, etc.). Items are managed in Settings with expandable rows, per-item parameter inputs, assignment list boxes, and viewport-driven object/surface assignment.
...
- Do not save files to, or import files from, paths outside this repository root (`/home/fiona-wang/Documents/ds_webapp`), unless the user explicitly requests an exception for a specific path.
...
| Click row text on "Power Source 1" | Row collapsed | Row expands to show power input + assignment list box + `+/-` controls | `bc_power_source_expanded.png` |
...
| Click "Temperature" | Power Source settings | Settings shows "Temperatures" title + "Add Temperature" button | `bc_temperature_two.png` |
| Click row text on "Temperature 1" | Row collapsed | Row expands to show temperature input + assignment list box + `+/-` controls | `bc_temperature_expanded.png` |
| Add two temperatures | Empty list | Both "Temperature 1" and "Temperature 2" visible | `bc_temperature_two.png` |
| Click object while Power Source row is expanded | No assignment | Object assigned/toggled in list; exclusivity across power sources enforced | `bc_power_source_assignment_exclusive.png` |
| Click CAD face while Temperature row is expanded | No assignment | Surface label `Object:Face-N` assigned/toggled; surface-only highlight and exclusivity enforced | `bc_temperature_surface_assignment_exclusive.png` |
...
- Per-item Power Source/Temperature inputs (`W/m`, `oC`) with default `0`
- Expandable per-item assignment panels (row-text clickable, `+/-`, fixed-height scroll list)
- Object assignment for Power Source and CAD-face assignment for Temperature (`Object:Face-N`)
- Global exclusivity validation (object-per-power-source, surface-per-temperature)
- Multi-select assignment row deletion (`single`, `Ctrl/Cmd`, `Shift`) with batch `-` removal
- Active-item highlight mirrors full assignment list (objects for power, surfaces for temperature)
...
6. **Visual image is the pass/fail source of truth** — do not mark a task done from state keys or logs alone when the screenshot still looks wrong.
7. **Failure capture is mandatory** — if a screenshot does not match expected behavior, store it as a failed verification artifact and report it explicitly as FAIL.
8. **No mixed pass/fail claims** — do not present a sequence as passed if any screenshot in that sequence fails visual inspection.
9. **Re-verify after each fix** — rerun the same sequence and capture a fresh full before/after set; only then change status to PASS.
10. **BC-to-Geometry specific check** — for boundary-condition highlight workflows, include this required visual check:
    - After switching from BC assignment mode to `Geometry`/`Import N`, BC highlight must be fully cleared before any new geometry selection.
